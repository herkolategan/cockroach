...
var (
	...
	flagVerbose   = flags.Bool("verbose", false, "log verbose output from roachprod commands to the terminal")
	...
	flagOutputDir = flags.String("outputdir", "", "directory to store the logs and results of the benchmarks")
	logOutputDir  string
	benchDir      string
	timestamp     time.Time
	testArgs      []string
...
func logResponseErrors(response remoteResponse) {
	if response.err != nil {
		l.Errorf("Remote command = {%s}, error = {%v}, stderr output = {%s}",
			strings.Join(response.command, " "), response.err, response.stderr)
	}
}
...
func run() error {
	...
	executeRemoteCommands(cluster, commands, numNodes, *flagFailFast, func(response remoteResponse) {
		fmt.Print(".")
