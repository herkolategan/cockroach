...
func remoteWorker(
	log *logger.Logger, clusterNode string, receive chan []RemoteCommand, response chan RemoteResponse, haltSignal *bool,
) {
	for {
		commands := <-receive
		if commands == nil {
...
func ExecuteRemoteCommands(
	...
	*haltSignal = false
	...
	for idx := 1; idx <= numNodes; idx++ {
		go remoteWorker(log, fmt.Sprintf("%s:%d", cluster, idx), workChannel, responseChannel, haltSignal)
	}
